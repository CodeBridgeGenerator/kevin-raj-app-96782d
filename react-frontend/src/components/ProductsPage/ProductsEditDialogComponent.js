    ...
    useEffect(() => {
     ...
     useEffect(() => {
                    ...
                    client
                        ...
                        .then((res) => {
                            setusers(res.data);
                            setcustomerId(res.data.map((e) => ({ name: e['name'], value: e._id })));
                        })
                        .catch((error) => {
                            console.log({ error });
    ...
    return (
        <Dialog header="Edit Info" visible={props.show} closable={false} onHide={props.onHide} modal style={{ width: "40vw" }} className="min-w-max" footer={renderFooter()} resizable={false}>
            <div role="products-edit-dialog-component">
                ...
                <Dropdown value={_entity?.customerId?._id} options={customerIdOptions} optionLabel="name" optionValue="value" onChange={(e) => setValByKey("customerId", e.value)} />
            </div>
